...
const LEAKED_KEY_MODULUS_PREFIX: [u8; 32] = [
    0xD4, 0x07, 0xE5, 0x13, 0x9B, 0x7A, 0x2C, 0x61,
    0xA8, 0x33, 0x02, 0xF9, 0x44, 0xBE, 0x55, 0xD7,
    0x8E, 0x6F, 0x21, 0xC3, 0x77, 0xAA, 0x09, 0xE8,
    0x50, 0x1B, 0x4D, 0x96, 0xCB, 0x63, 0xF2, 0x38,
];
...
impl MsiKeyReuseDetector {
    ...
    fn check_boot_guard_with_weak_signature(&self, data: &[u8]) -> Vec<Finding> {
        ...
        if has_km && has_bpm {
            ...
            let has_pkcs1_sig = data.windows(3).any(|w| {
                w[0] == 0x00 && w[1] == 0x01 && w[2] == 0xFF
            });
